...
try:
    ...
    print("Установка библиотеки pyTelegramBotAPI...")
    subprocess.check_call([sys.executable, "-m", "pip", "install", "pyTelegramBotAPI==4.26.0"])
    import telebot
...
TOKEN = os.getenv(
    "BOT_TOKEN",
    "8924716382:AAEwSTDZ7uipfNGypnj2v8yZGshY_1tvfyI",
)
...
def init_db():
    ...
    if "last_bonus" not in columns:
        cursor.execute(
            "ALTER TABLE users ADD COLUMN last_bonus REAL DEFAULT 0.0"
        )
...
def get_user(user):
    ...
    cursor.execute(
        """
        SELECT balance, bet, last_action, last_bonus
        FROM users
        WHERE user_id = ?
        """,
        (user.id,),
    ...
    if not row:
        cursor.execute(
            """
            INSERT INTO users
            (user_id, balance, bet, name, last_action, last_bonus)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
...
def update_user_data(
    ...
    if bet is not None:
        cursor.execute(
            "UPDATE users SET bet = ? WHERE user_id = ?",
            (bet, user_id),
        )
...
def get_games_menu():
    ...
    markup.add(
        ...
        InlineKeyboardButton(
            "🎲 Кубик (х3 за 6, х1.5 за 5)",
            callback_data="game_dice",
        ),
        InlineKeyboardButton(
            "🎯 Дартс (х3.5 яблочко, х1.5 рядом)",
            callback_data="game_darts",
        ),
        InlineKeyboardButton(
            "🎳 Боулинг (х4 страйк, х2 почти)",
            callback_data="game_bowling",
        ),
        InlineKeyboardButton(
            "🏀 Баскетбол (х2.5 попадание, х1.5 обод)",
...
def get_shop_menu():
    ...
    markup.add(
        InlineKeyboardButton(
            "⭐ 50 монет — 6 ⭐ (Старт)",
            callback_data="buy_50_6",
        ),
        InlineKeyboardButton(
            "🌟 300 монет — 15 ⭐ (Выгодно)",
            callback_data="buy_300_15",
        ),
        InlineKeyboardButton(
            "🥉 1000 монет — 50 ⭐",
            callback_data="buy_1000_50",
        ),
        InlineKeyboardButton(
            "🥈 5000 монет — 200 ⭐",
            callback_data="buy_5000_200",
        ),
        InlineKeyboardButton(
            "🥇 15000 монет — 500 ⭐ (Топ!)",
            callback_data="buy_15000_500",
        ),
    )
...
def handle_text(message):
    ...
    if text == "🎰 ИГРАТЬ":
        bot.send_message(
            message.chat.id,
            f"🎰 Выбирай игру!\n"
            f"🎲 Ваша текущая ставка: {u_data['bet']} 🪙",
        ...
        bot.send_message(
            message.chat.id,
            f"⚙️ Ваша ставка: {u_data['bet']} 🪙.\n"
            f"Выберите новую сумму:",
            reply_markup=get_bet_menu(),
        )

    elif text == "💰 Баланс":
        bot.send_message(
            message.chat.id,
            f"💳 Ваш баланс: {u_data['balance']} 🪙\n"
            f"🎲 Текущая ставка: {u_data['bet']} 🪙",
        )
        ...
        bot.send_message(
            message.chat.id,
            "🛒 Выберите пакет монет для покупки "
            "за Telegram Stars (XTR):",
...
def handle_withdraw(call):
    ...
    if bal < cost:
        bot.answer_callback_query(
            call.id,
            f"❌ Недостаточно монет! Для вывода {stars} ⭐ "
            f"нужно {cost} 🪙.\nВаш баланс: {bal} 🪙.",
            show_alert=True,
        )
    else:
        ...
        bot.send_message(
            ...
            f"Ждите, ваша заявка скоро будет принята.\n\n"
            f"💳 Списано: {cost} 🪙\n"
            f"💰 Остаток: {new_bal} 🪙",
            parse_mode="Markdown",
        )
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith("setbet_")
)
...
def handle_buy(call):
    ...
    bot.send_invoice(
        ...
        description=(
            f"Покупка {coins} монет за "
            f"{stars} Telegram Stars (XTR)."
        ),
...
def handle_game(call):
    ...
    if time_passed < GAME_COOLDOWN:
        bot.answer_callback_query(
            call.id,
            f"⏳ Подождите еще "
            f"{round(GAME_COOLDOWN - time_passed, 1)} сек! "
            f"Анимация еще не закончилась.",
            show_alert=True,
        )
        return
    ...
    if bet <= 0:
        bot.answer_callback_query(
            call.id,
            "❌ Ваша ставка 0. Увеличьте её в меню!",
            show_alert=True,
        )
        return
    ...
    if game_type == "slot":
        if val == 64:
            mult = 50.0
            text_result = "🎰 ДЖЕКПОТ! 777!"
        elif val in [1, 22, 43]:
            mult = 12.0
            text_result = "🎉 Три одинаковых!"
        elif val in [16, 32, 48]:
            mult = 1.5
            text_result = "✨ Пара символов!"
        else:
            text_result = "😢 Увы, не повезло."

    elif game_type == "dice":
        if val == 6:
            mult = 3.0
            text_result = "🎲 Выпала 6! Выигрыш x3!"
        elif val == 5:
            mult = 1.5
            text_result = "🎲 Выпала 5! Выигрыш x1.5!"
        else:
            text_result = f"🎲 Выпало {val}. Проигрыш."

    elif game_type == "darts":
        if val == 6:
            mult = 3.5
            text_result = "🎯 ЯБЛОЧКО! (x3.5)"
        elif val == 5:
            mult = 1.5
            text_result = "🎯 Почти в центр! (x1.5)"
        else:
            text_result = "🎯 Промах."
        ...
        if val == 6:
            mult = 4.0
            text_result = "🎳 СТРАЙК! (x4)"
        elif val == 5:
            mult = 2.0
            text_result = "🎳 Почти все кегли! (x2)"
        else:
            text_result = "🎳 Мимо кеглей."

    elif game_type == "basketball":
        if val == 5:
            mult = 2.5
            text_result = "🏀 ТОЧНЫЙ БРОСОК В СЕТКУ! (x2.5)"
        elif val == 4:
            mult = 1.5
            text_result = "🏀 Попадание от дуги! (x1.5)"
        else:
            text_result = "🏀 Мяч не попал в корзину."
    ...
    final_text += (
        f"💳 Ваш текущий баланс: {final_bal} 🪙"
    )
